...
//	SET_PERI_REG_MASK(SLC_TX_LINK, ((uint32)&i2sBufDescZeroes) & SLC_TXLINK_DESCADDR_MASK); //any random desc is OK, we don't use TX but it needs something valid
	...
	SET_PERI_REG_MASK(SLC_RX_LINK,
			((uint32)&i2sBufDescOut) & SLC_RXLINK_DESCADDR_MASK);
...
//	CLEAR_PERI_REG_MASK(I2SCONF_CHAN, (I2S_TX_CHAN_MOD<<I2S_TX_CHAN_MOD_S)|(I2S_RX_CHAN_MOD<<I2S_RX_CHAN_MOD_S));
	...
	SET_PERI_REG_MASK(I2SCONF,
			I2S_RIGHT_FIRST|I2S_MSB_RIGHT|I2S_RECE_SLAVE_MOD| I2S_RECE_MSB_SHIFT|I2S_TRANS_MSB_SHIFT| (((WS_I2S_BCK)&I2S_BCK_DIV_NUM )<<I2S_BCK_DIV_NUM_S)| (((WS_I2S_DIV)&I2S_CLKM_DIV_NUM)<<I2S_CLKM_DIV_NUM_S));
	...
	SET_PERI_REG_MASK(I2SINT_ENA,
			I2S_I2S_RX_REMPTY_INT_ENA|I2S_I2S_RX_TAKE_DATA_INT_ENA);
...
static const uint16_t bitpatterns[16] = { 0b100100100100, 0b100100100110,
		0b100100110100, 0b100100110110, 0b100110100100, 0b100110100110,
		0b100110110100, 0b100110110110, 0b110100100100, 0b110100100110,
		0b110100110100, 0b110100110110, 0b110110100100, 0b110110100110,
		0b110110110100, 0b110110110110, };
...
//	if( buffersize * 4 + 1 > WS_BLOCKSIZE ) return;
	...
	for (place = 0; !quit; place++) {
		uint8_t b;
		b = buffer[pl++];
		uint16_t c1a = bitpatterns[(b & 0x0f)];
		uint16_t c1b = bitpatterns[(b >> 4)];
		b = buffer[pl++];
		uint16_t c2a = bitpatterns[(b & 0x0f)];
		uint16_t c2b = bitpatterns[(b >> 4)];
		b = buffer[pl++];
		uint16_t c3a = bitpatterns[(b & 0x0f)];
		uint16_t c3b = bitpatterns[(b >> 4)];
		b = buffer[pl++];
		uint16_t c4a = bitpatterns[(b & 0x0f)];
		uint16_t c4b = bitpatterns[(b >> 4)];
